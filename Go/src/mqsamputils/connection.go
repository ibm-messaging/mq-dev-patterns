...
func getEndPoint(index int) Env {
	...
	if len(MQ_ENDPOINTS.Points) > 0 && len(MQ_ENDPOINTS.Points) >= index {
...
func ccdtCheck() bool {
	...
	if fPath := os.Getenv(CCDT); "" != fPath {
		// Open the file to check it's accessible. If so, we close it immediately anyway
...
func CreateConnection(index int) (ibmmq.MQQueueManager, error) {
	...
	if !ccdtCheck() {
		logger.Println("CCDT URL export is not set, will be using json environment client connection settings")
		// Make the CNO refer to the CSP structure returned, so it gets used during the connection
...
func OpenDynamicQueue(qMgrObject ibmmq.MQQueueManager, queueName string) (ibmmq.MQObject, error) {
	return openObject(qMgrObject, queueName, Response, FULL_STRING)
}

func OpenReplyQueue(qMgrObject ibmmq.MQQueueManager, queueName string) (ibmmq.MQObject, error) {
	return openObject(qMgrObject, queueName, Response, FULL_STRING)
}
...
func OpenGetQueue(qMgrObject ibmmq.MQQueueManager, msgStyle string, index int) (ibmmq.MQObject, error) {
	return openObject(qMgrObject, "", msgStyle, index)
}

// Opens an object - a queue or topic. No name is directly provided; it comes from the environment
func OpenObject(qMgrObject ibmmq.MQQueueManager, msgStyle string) (ibmmq.MQObject, error) {
	return openObject(qMgrObject, "", msgStyle, FULL_STRING)
}

// Internal function to allow it to be modified, without affecting the callers.
func openObject(qMgrObject ibmmq.MQQueueManager, replyToQ string, msgStyle string, index int) (ibmmq.MQObject, error) {
	...
	case Put:
		...
		logger.Printf("Attempting to open queue %s", env.QueueName)
	...
	case Pub:
		...
		logger.Printf("Attempting to open topic %s", env.Topic)

	case Dynamic:
		openOptions = ibmmq.MQOO_INPUT_EXCLUSIVE
		mqod.ObjectName = env.ModelQueueName
		mqod.DynamicQName = env.DynamicQueueName
		logger.Printf("Attempting to open queue %s", env.ModelQueueName)

	case Response:
		mqod.ObjectType = ibmmq.MQOT_Q
		mqod.ObjectName = replyToQ
		logger.Printf("Attempting to open queue %s", replyToQ)
	}
