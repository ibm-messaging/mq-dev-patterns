    ...
    return new Promise((resolve, reject) => {
      ...
      this.mqclient
        ...
        .then((hObj) => {
          this.hObjQueue = hObj;
          debug_info(
            `Subscriber ${this.myID} starting background message poller`
          );
  ...
  deleteSubscription() {
    ...
    return new Promise((resolve, reject) => {
      this.mqclient
        ...
        .catch((err) => {
          debug_warn(
            `Subscriber ${this.myID} error on connection close ${err}`
          );
          reject();
        });
    });
  ...
  pullingTheQueue() {
    ...
    setTimeout(
      () => {
        ...
        this.mqclient
          .performGet(1, this.hObjQueue)
          .then(async (message) => {
            debug_info(`Subscriber ${pubID} poll ${myID} message received`);
            if (message && message.length > 0) {
              debug_info(
                `Subscriber ${pubID} poll ${myID} stashing message ${message}`
              );
              let messageStashedOK = await this.messages.push(message);
              if (!messageStashedOK) {
                debug_warn(
                  `Subscriber ${pubID} poll ${myID} detected overgrown message stash`
                );
                this.warningCount++;
                if (MAX_STASH_WARNINGS < this.warningCount) {
                  debug_warn(
                    `Subscriber ${pubID} poll ${myID} Exceeded warning count`
                  );
                  this.killThePoll();
                }
              }
