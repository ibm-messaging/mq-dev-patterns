...
class MQClient {
      ...
      mutexDynamicVariablePut
        ...
        .then(() => {
          ...
          if (this._DYNAMIC === 'DYNPUT') {
            ...
            return this.performConnection(putRequest.queueName).then(() => {
              this.performPutWithSessionID(msg, 1, sessionID);
            });
          } else if (this.currency) {
            debug_info(
              'Connected to MQ for put with properties ' + this.currency
            );
  ...
  sub(topic) {
    return new Promise((resolve, reject) => {
      let releaseFunction = null;
      debug_info(
        `mqclient ${this.myID} aquiring lock for sub on topic ${topic}`
      );
      mutexTopicVariableSub
        .acquire()
        .then((release) => {
          releaseFunction = release;
          this._TOPIC = topic;
        ...
        .then((hObj) => {
          if (releaseFunction) {
            debug_info(
              `mqclient ${this.myID} releasing lock for sub on topic ${topic}`
            );
            releaseFunction();
          }
          resolve(hObj);
        ...
        .catch((err) => {
          debug_warn(
            `mqclient ${this.myID} failed on subscription for ${topic}, error ${err}`
          );
          if (releaseFunction) {
            releaseFunction();
          }
  ...
  pub(topic, quantity, message) {
    ...
    this._TOPIC = topic;
    debug_info(
      `mqclient ${this.myID} publishing ${quantity} messages to ${topic}`
    );
    return new Promise((resolve, reject) => {
      this.makeConnectionPromise(topic)
        .then(() => {
          if (!this.isReady()) {
            debug_warn(
              `aborting put as mqclient ${this.myID} is not properly connected for pub on topic ${topic}`
            );
            reject('Connection Error on pub');
          } else {
            debug_info(
              `mqclient ${this.myID} connected for pub on topic ${topic}`
            );
            return this.performPut(message, quantity);
          }
        })
    ...
    return new Promise((resolve, reject) => {
      ...
      this.buildCNO()
        ...
        .then((hconn) => {
          ...
          if (null == hconn) {
            debug_warn(
              `mqclient ${this.myID} hconn error in performConnection`
            );
          ...
          if (this._TOPIC) {
            ...
            debug_info(
              `mqclient ${this.myID} opening connection for Dynamic connection`
            );
        ...
        .catch((err) => {
          debug_info(
            `mqclient ${this.myID} error on perform connection ${err} `
          );
          this._HEALTHY = false;
          reject(err);
        });
  ...
  performOpen(_QUEUE_NAME) {
    ...
    if (this.sessionID !== null) {
      ...
      debug_info(
        `The selection string for this responde request is ${selectionString}`
      );
  ...
  makeConnectionPromise(queueOrTopic = null) {
    ...
    if (
      this[_HCONNKEY] == null ||
      (this[_HOBJKEY] == null && this[_HOBJDYN] == null)
    ) {
      debug_info(`mqclient ${this.myID} creating new connection`);
      connectionPromise = this.performConnection(queueOrTopic);
    } else {
      debug_info(
        `mqclient ${this.myID} connection already exists for HCONNKEY ${this[_HCONNKEY]} and HOBJKEY ${this[_HOBJKEY]}`
      );
    }
  ...
  performPutWithProperites(message, quantity) {
    return new Promise((resolve, reject) => {
      resolve(
        'You should replace this promise with the PUT function that will set the currency as a message property'
      );
    });
  }
  //=======================================================================
  ...
  performPut(message, quantity) {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      if (mqmd == null || pmo == null) {
        debug_warn(
          `mqclient ${this.myID} unable to create mqmd or pmo for pub call`
        );
        break;
      }
      ...
      if (quantity === 1) {
        debug_info(
          `mqclient ${this.myID} Only a single promise to return. HOBJKEY is ${this[_HOBJKEY]}`
        );
        return mq.PutPromise(this[_HOBJKEY], mqmd, pmo, msg);
      }
    ...
    if (promises.length == 0) {
      debug_warn(
        `mqclient ${this.myID} returning empty publish promises array`
      );
    }
  ...
  performSub(topic) {
    return new Promise(async (resolve, reject) => {
      ...
      mutexMQClientPerformSub.acquire().then((release) => {
        mq.Sub(
          me[_HCONNKEY],
          null,
          sd,
          function (err, hObjPubQ, hObjSubscription) {
            if (err) {
              release();
              reject(err);
            } else {
  ...
  getSomeMessages(limit, hObj) {
    return new Promise((resolve, reject) => {
      ...
      let getPromise = this.currency
        ? this.getSingleMessageWithProperties()
      ...
      getPromise
        .then((messageData) => {
          ...
          if (obtainedMessages.length >= limit || messageData == null) {
            debug_info(
              `mqclient ${this.myID} resolving full array or no more messages`
            );
  ...
  getSingleMessage(hObj) {
    return new Promise((resolve, reject) => {
      if (!this._HEALTHY) {
        debug_info(
          `mqclient ${this.myID} is no longer healthy aborting getSingleMessage`
        );
      }
      let _dyn = this._DYNAMIC;
      ...
      mq.GetSync(!hObj ? this[_HOBJKEY] : hObj, mqmd, gmo, buf, (err, len) => {
        if (err) {
          if (err.mqrc === MQC.MQRC_NO_MSG_AVAILABLE) {
            ...
            debug_warn(
              `mqclient ${this.myID} for logEntry ${logEntry} error retrieving message ${err}`
            );
            this._HEALTHY = false;
          }
          debug_info(
            `mqclient ${this.myID} resolving null as no message found`
          );
          resolve(null);
  ...
  getRESTConfiguration() {
    let RESTCredential = {
      ...
      MQ_QMGR_PORT_API:
        process.env['MQ_QMGR_PORT_API'] ||
        MQDetails.MQ_QMGR_PORT_API ||
        DEFAULT_MQ_HTTP_PORT,
    };
