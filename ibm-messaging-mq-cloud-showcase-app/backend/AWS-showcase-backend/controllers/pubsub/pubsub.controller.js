...
async function sub(req, res, next) {
  ...
  if (subscriber !== -1) {
    ...
    subscriber.deleteSubscription().then(async () => {
      subscriber
...
async function getLastSubMessage(req, res, next) {
  ...
  if (subscriber !== -1) {
    ...
    subscriber.getMessages(1).then((_lastMessage) => {
...
async function pub(req, res, next) {
  ...
  if (publisher !== -1 && !isPublishing) {
    ...
    //Publish the message
    publisher
      .publishMessages(topic, quantity, _message)
      .then((pubRes) => {
    ...
    //if there are not publishers and we are not in a middle of a publishing transaction
